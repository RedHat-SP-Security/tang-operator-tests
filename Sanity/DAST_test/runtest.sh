...
rlPhaseStartTest "Dynamic Application Security Testing"
    ...
    DEFAULT_TOKEN=""
    if [ -n "${KONFLUX}" ]; then
        # ephemeral Konflux: multiple fallbacks + placeholder
        [ -n "${KUBECONFIG}" ] && [ -f "${KUBECONFIG}" ] && DEFAULT_TOKEN=$("${OC_CLIENT}" --kubeconfig="${KUBECONFIG}" whoami -t 2>/dev/null || true)
        [ -z "${DEFAULT_TOKEN}" ] && DEFAULT_TOKEN=$("${OC_CLIENT}" sa get-token "${OPERATOR_NAME}" -n "${OPERATOR_NAMESPACE}" 2>/dev/null || true)
        if [ -z "${DEFAULT_TOKEN}" ]; then
            SECRET_NAME=$("${OC_CLIENT}" get sa "${OPERATOR_NAME}" -n "${OPERATOR_NAMESPACE}" -o jsonpath='{.secrets[0].name}' 2>/dev/null || true)
            [ -n "${SECRET_NAME}" ] && DEFAULT_TOKEN=$("${OC_CLIENT}" get secret "${SECRET_NAME}" -n "${OPERATOR_NAMESPACE}" -o jsonpath='{.data.token}' | base64 --decode || true)
        fi
        [ -z "${DEFAULT_TOKEN}" ] && DEFAULT_TOKEN="TOKEN_DOESNT_NEED_IN_EPHEMERAL"
    else
        # CRC / persistent: try 'oc create token' first
